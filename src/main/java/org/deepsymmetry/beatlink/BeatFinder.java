...
/**
 * <p>
 * Watches for devices to report new beats by broadcasting beat packets on port
 * 50001, and passes them on to registered listeners. When players are actively
 * playing music, they send beat packets at the start of each beat which, in
 * addition to announcing the start of the beat, provide information about where
 * the beat falls within a measure of music (assuming that the track was
 * properly configured in rekordbox, and is in 4/4 time), the current BPM of the
 * track being played, and the current player pitch adjustment, from which the
 * actual effective BPM can be calculated.
 * </p>
 *
 * <p>
 * When players are stopped, they do not send beat packets, but the mixer
 * continues sending them at the last BPM reported by the master player, so it
 * acts as the most reliable synchronization source. The mixer does not make any
 * effort to keep its notion of measures (down beats) consistent with any
 * player, however. So systems which want to stay in sync with measures as well
 * as beats will want to use the {@link VirtualCdj} to maintain awareness of
 * which player is the master player.
 * </p>
 *
 * This class also receives special sync and on-air control messages which are
 * sent to the same port as beat packets. If the {@link VirtualCdj} is sending
 * status packets, it needs to be notified about these so it can properly update
 * its sync and on-air state if the mixer tells it to, so it will ensure the
...
public class BeatFinder extends LifecycleParticipant {
	 ...
	 *
	 * @return {@code true} if our socket is open and monitoring for DJ Link beat
	...
	/**
	 * Keeps track of the warnings we have already issued for packets of a given
	 * type with a given unexpected size, so we only issue them once.
	 */
	private static final Map<String, Set<Integer>> sizeWarningsIssued = new HashMap<String, Set<Integer>>();

	/**
	 * Checks whether this is the first time we have seen a particular bad size for
	 * a given packet type, and returns {@code true} in that circumstance, making
	 * note that we have now seen this combination, so we will return {@code false}
	 * for it in the future.
	 *
	 * @param packetName       the type of packet to be warned about
	 * @param unexpectedLength the number of bytes in this packet, which we did not
	 *                         expect to see
	 *
	 ...
	 * @param name           the description of the packet in case we need to report
	...
	private boolean isPacketLongEnough(DatagramPacket packet, int expectedLength, String name) {
		...
		if (length > expectedLength && shouldWarn(name, length)) {
			logger.warn("Processing too-long " + name + " packet; expecting " + expectedLength + " bytes and got "
					+ length + ".");
		}
	...
	/**
	 * Start listening for beat announcements and sync commands. If already
	 * listening, has no effect.
	 *
	 * @throws SocketException if the socket to listen on port 50001 cannot be
	 *                         created
	 */
				...
				public void run() {
					...
					while (isRunning()) {
						try {
							...
							// Don't log a warning if the exception was due to the socket closing at
							// shutdown.
							if (isRunning()) {
							...
							if (received) {
								...
								if (kind != null) {
									...
									case ABSOLUTE_POSITION:
										if (isPacketLongEnough(packet, 60, "absolute position")) {
											deliverAbsolutePosition(new AbsolutePosition(packet));
										}
										break;
									...
									default:
										logger.warn(
												"Ignoring packet received on beat port with unexpected type: " + kind);
									}
								}
							}
						} catch (Throwable t) {
	...
	protected void deliverAbsolutePosition(AbsolutePosition absolutePosition) {
//		VirtualCdj.getInstance().processBeat(beat);
		final AbsolutePositionListener timeFinderListener = timeFinderAbsolutePositionListener.get();
		if (timeFinderListener != null) {
			try {
				timeFinderListener.newAbsolutePosition(absolutePosition);
			} catch (Throwable t) {
				logger.warn("Problem delivering absolute position announcement to TimeFinder listener", t);
			}
		}
		for (final AbsolutePositionListener listener : new LinkedList<AbsolutePositionListener>(absolutePositionListeners)) {
			try {
				listener.newAbsolutePosition(absolutePosition);
			} catch (Throwable t) {
				logger.warn("Problem delivering absolute position announcement to listener", t);
			}
		}
	}
	...
	/**
	 * Keeps track of the TimeFinder's beat listener when it is registered, so we
	 * can call it first.
	 */
	private final AtomicReference<BeatListener> timeFinderBeatListener = new AtomicReference<BeatListener>();

	/**
	 * Keeps track of the TimeFinder's absolute beat listener when it is registered,
	 * so we can call it first.
	 */
	private final AtomicReference<AbsolutePositionListener> timeFinderAbsolutePositionListener = new AtomicReference<AbsolutePositionListener>();

	/**
	 * Keeps track of the registered beat listeners, except the TimeFinder's.
	 */
	private final Set<BeatListener> beatListeners = Collections
			.newSetFromMap(new ConcurrentHashMap<BeatListener, Boolean>());

	/**
	 * <p>
	 * Adds the specified beat listener to receive beat announcements when DJ Link
	 * devices broadcast them on the network. If {@code listener} is {@code null} or
	 * already present in the list of registered listeners, no exception is thrown
	 * and no action is performed.
	 * </p>
	 *
	 * <p>
	 * To reduce latency, beat announcements are delivered to listeners directly on
	 * the thread that is receiving them them from the network, so if you want to
	 * interact with user interface objects in listener methods, you need to use
	 * <code><a href=
	 * "http://docs.oracle.com/javase/8/docs/api/javax/swing/SwingUtilitbeatListenersies.html#invokeLater-java.lang.Runnable-">javax.swing.SwingUtilities.invokeLater(Runnable)</a></code>
	 * to do so on the Event Dispatch Thread.
	 *
	 * Even if you are not interacting with user interface objects, any code in the
	 * listener method <em>must</em> finish quickly, or it will add latency for
	 * other listeners, and beat announcements will back up. If you want to perform
	 * lengthy processing of any sort, do so on another thread.
	 * </p>
	 *
	...
	/**
	 * Removes the specified beat listener so that it no longer receives beat
	 * announcements when DJ Link devices broadcast them to the network. If
	 * {@code listener} is {@code null} or not present in the list of registered
	 ...
	 */
	private final Set<AbsolutePositionListener> absolutePositionListeners = Collections
			.newSetFromMap(new ConcurrentHashMap<AbsolutePositionListener, Boolean>());

	public void addAbsolutePositionListener(AbsolutePositionListener listener) {
		if (TimeFinder.getInstance().isOwnAbsolutePositionListener(listener)) {
			timeFinderAbsolutePositionListener.set(listener);
		} else if (listener != null) {
			absolutePositionListeners.add(listener);
		}
	}
	 ...
	 * @param listener the beat listener to remove
	 */
	public void removeAbsolutePositionListener(AbsolutePositionListener listener) {
		if (TimeFinder.getInstance().isOwnAbsolutePositionListener(listener)) {
			timeFinderAbsolutePositionListener.set(null);
		} else if (listener != null) {
			absolutePositionListeners.remove(listener);
		}
	}
	 ...
	 */
	public Set<AbsolutePositionListener> getAbsolutePositionListeners() {
		// Make a copy so callers get an immutable snapshot of the current state.
		final Set<AbsolutePositionListener> result = new HashSet<AbsolutePositionListener>(absolutePositionListeners);
		final AbsolutePositionListener timeFinderListener = timeFinderAbsolutePositionListener.get();
		if (timeFinderListener != null) {
	...
	/**
	 * Send a beat announcement to all registered listeners, and let the
	 * {@link VirtualCdj} know about it in case it needs to notify the master beat
	 * listeners.
	 *
	 ...
	 * Keeps track of the registered sync command listeners.
	 */
	private final Set<SyncListener> syncListeners = Collections
			.newSetFromMap(new ConcurrentHashMap<SyncListener, Boolean>());

	/**
	 * <p>
	 * Adds the specified sync command listener to receive sync commands when DJ
	 * Link devices send them to Beat Link. If {@code listener} is {@code null} or
	 * already present in the list of registered listeners, no exception is thrown
	 * and no action is performed.
	 * </p>
	 *
	 * <p>
	 * To reduce latency, sync commands are delivered to listeners directly on the
	 * thread that is receiving them them from the network, so if you want to
	 * interact with user interface objects in listener methods, you need to use
	 * <code><a href=
	...
	/**
	 * Removes the specified sync listener so that it no longer receives sync
	 * commands when DJ Link devices send them to Beat Link. If {@code listener} is
	 * {@code null} or not present in the list of registered listeners, no exception
	 ...
	 */
	private final Set<MasterHandoffListener> masterHandoffListeners = Collections
			.newSetFromMap(new ConcurrentHashMap<MasterHandoffListener, Boolean>());

	/**
	 * <p>
	 * Adds the specified master handoff listener to receive tempo master handoff
	 * commands when DJ Link devices send them to Beat Link. If {@code listener} is
	 * {@code null} or already present in the list of registered listeners, no
	 * exception is thrown and no action is performed.
	 * </p>
	 *
	 * <p>
	 * To reduce latency, handoff commands are delivered to listeners directly on
	 * the thread that is receiving them them from the network, so if you want to
	 * interact with user interface objects in listener methods, you need to use
	 * <code><a href=
	 * "http://docs.oracle.com/javase/8/docs/api/javax/swing/SwingUtilities.html#invokeLater-java.lang.Runnable-">javax.swing.SwingUtilities.invokeLater(Runnable)</a></code>
	...
	/**
	 * Removes the specified master handoff listener so that it no longer receives
	 * tempo master handoff commands when DJ Link devices send them to Beat Link. If
	 * {@code listener} is {@code null} or not present in the list of registered
	 * listeners, no exception is thrown and no action is performed.
	 *
	...
	/**
	 * Get the set of master handoff command listeners that are currently
	 * registered.
	 *
	 ...
	 *
	 * @param toPlayer the device number to which we are being instructed to yield
	 ...
	 *
	 * @param fromPlayer the device number that is responding to our request that it
	 *                   yield the tempo master role to us
	 ...
	 */
	private final Set<OnAirListener> onAirListeners = Collections
			.newSetFromMap(new ConcurrentHashMap<OnAirListener, Boolean>());

	/**
	 * <p>
	 * Adds the specified on-air listener to receive channel on-air updates when the
	 * mixer broadcasts them on the network. If {@code listener} is {@code null} or
	 * already present in the list of registered listeners, no exception is thrown
	 * and no action is performed.
	 * </p>
	 *
	 * <p>
	 * To reduce latency, on-air updates are delivered to listeners directly on the
	 * thread that is receiving them them from the network, so if you want to
	 * interact with user interface objects in listener methods, you need to use
	 * <code><a href=
	 * "http://docs.oracle.com/javase/8/docs/api/javax/swing/SwingUtilities.html#invokeLater-java.lang.Runnable-">javax.swing.SwingUtilities.invokeLater(Runnable)</a></code>
	...
	/**
	 * Removes the specified on-air listener so that it no longer receives channel
	 * on-air updates when the mixer broadcasts them to the network. If
	 * {@code listener} is {@code null} or not present in the list of registered
	 * listeners, no exception is thrown and no action is performed.
	 *
	 ...
	 *
	 * @param audibleChannels holds the device numbers of all channels that can
	 *                        currently be heard in the mixer output
	 */
	 ...
	 */
	private final Set<FaderStartListener> faderStartListeners = Collections
			.newSetFromMap(new ConcurrentHashMap<FaderStartListener, Boolean>());

	/**
	 * <p>
	 * Adds the specified fader start listener to receive fader start commands when
	 * the mixer broadcasts them on the network. If {@code listener} is {@code null}
	 * or already present in the list of registered listeners, no exception is
	 * thrown and no action is performed.
	 * </p>
	 *
	 * <p>
	 * To reduce latency, fader start commands are delivered to listeners directly
	 * on the thread that is receiving them them from the network, so if you want to
	 * interact with user interface objects in listener methods, you need to use
	 * <code><a href=
	 * "http://docs.oracle.com/javase/8/docs/api/javax/swing/SwingUtilities.html#invokeLater-java.lang.Runnable-">javax.swing.SwingUtilities.invokeLater(Runnable)</a></code>
	 * to do so on the Event Dispatch Thread.
	...
	/**
	 * Removes the specified fader start listener so that it no longer receives
	 * fader start commands when the mixer broadcasts them to the network. If
	 * {@code listener} is {@code null} or not present in the list of registered
	 * listeners, no exception is thrown and no action is performed.
	 *
	 ...
	 *
	 * @param playersToStart contains the device numbers of all players that should
	 *                       start playing
	 * @param playersToStop  contains the device numbers of all players that should
	 *                       stop playing
	 */
